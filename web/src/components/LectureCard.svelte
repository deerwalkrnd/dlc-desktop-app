<script lang="ts">
	...
	let currentVideoUrl = '';
	let currentLessonName = $state('');

	onMount(() => {
		dialog = document.getElementById('confirmation-dialog') as HTMLDialogElement;
		videoElement = document.querySelector('#video-player') as HTMLVideoElement;
		Controls();
	...
	const showDialogClick = (lessonName: string, videoUrl: string, asModal = true) => {
		try {
			...
			if (videoElement) {
				...
				if (sourceElement) {
					sourceElement.src = `${MEDIAURL}/videos/${videoUrl}`;
					videoElement.load();
				}
			}
...
{#snippet Lesson(lessonId: number, lessonName: string, lessonNumber: number, videoUrl: string)}
	<div class="rounded-md bg-indigo-50 p-3 transition-colors duration-200 hover:bg-indigo-100">
		<button
			...
			onclick={() => showDialogClick(`Lesson ${lessonNumber} : ${lessonName}`, videoUrl, true)}
			onkeydown={(e) =>
				e.key === 'Enter' &&
				showDialogClick(`Lesson ${lessonNumber} : ${lessonName}`, videoUrl, true)}
		>
...
>
	<div class="flex flex-col">
		<div class="mb-4 flex items-center justify-between">
			<h2 class="text-xl font-bold text-white">{currentLessonName}</h2>
				...
				>
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center justify-center gap-10">
							...
							>
								<i class="material-icons inline-block text-center text-5xl text-white">play_arrow</i
								>
							</button>
							...
							<input
								class="my-auto flex cursor-pointer appearance-none items-center justify-center rounded-xl bg-gray-200"
								type="range"
								id="volumeSlider"
								min="0"
								max="1"
